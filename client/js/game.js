...
function startGame(token, username) {
  console.log('Starting game for:', username); // Debug
  document.getElementById('login-ui').classList.add('hidden');
  document.getElementById('game-ui').classList.remove('hidden');
  networkManager = new NetworkManager(token, username);
...
document.getElementById('login-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (response.ok) {
      startGame(data.token, data.username);
    } else {
...
document.getElementById('register-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      document.getElementById('login-message').textContent = data.error || 'Registration failed';
    }
  } catch (error) {
